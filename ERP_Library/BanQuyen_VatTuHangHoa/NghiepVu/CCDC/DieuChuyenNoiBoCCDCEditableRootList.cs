...
{
    [Serializable()]
    public class DieuChuyenNoiBoCCDCList : Csla.BusinessListBase<DieuChuyenNoiBoCCDCList, DieuChuyenNoiBoCCDC>
    {
        ...
        private DieuChuyenNoiBoCCDCList()
        { /* require use of factory method */ }

        public static DieuChuyenNoiBoCCDCList NewDieuChuyenNoiBoCongCuDungCuList()
        {
            if (!CanAddObject())
                throw new System.Security.SecurityException("User not authorized to add a DieuChuyenNoiBoCongCuDungCuList");
            return new DieuChuyenNoiBoCCDCList();
        }
        public static DieuChuyenNoiBoCCDCList GetDieuChuyenNoiBoCongCuDungCuList()
        {
            if (!CanGetObject())
                throw new System.Security.SecurityException("User not authorized to view a DieuChuyenNoiBoCongCuDungCuList");
            return DataPortal.Fetch<DieuChuyenNoiBoCCDCList>(new FilterCriteriaAll());
        }
        public static DieuChuyenNoiBoCCDCList GetDieuChuyenNoiBoCongCuDungCuList_TimTheoNgay(DateTime ngay, DateTime denNgay)
        {
            if (!CanGetObject())
                throw new System.Security.SecurityException("User not authorized to view a DieuChuyenNoiBoCongCuDungCuList");
            return DataPortal.Fetch<DieuChuyenNoiBoCCDCList>(new FilterCriteria_TimTheoNgay(ngay, denNgay));
        }

        public static DieuChuyenNoiBoCCDCList GetDieuChuyenNoiBoCongCuDungCuList_TimTheoSoChungTu(String soChungTu, String phepToanSoSanh)
        {
            if (!CanGetObject())
                throw new System.Security.SecurityException("User not authorized to view a DieuChuyenNoiBoCongCuDungCuList");
            return DataPortal.Fetch<DieuChuyenNoiBoCCDCList>(new FilterCriteria_TimTheoSoChungTu(soChungTu, phepToanSoSanh));
        }
        public static DieuChuyenNoiBoCCDCList GetDieuChuyenNoiBoCongCuDungCuList(int maBoPhanGiao, long maNhanVienGiao, int maBoPhanNhan, long maNhanVienNhan, int maNguoiLap)
        {
            if (!CanGetObject())
                throw new System.Security.SecurityException("User not authorized to view a DieuChuyenNoiBoCongCuDungCuList");
            return DataPortal.Fetch<DieuChuyenNoiBoCCDCList>(new FilterCriteria(maBoPhanGiao, maNhanVienGiao, maBoPhanNhan, maNhanVienNhan, maNguoiLap));
        }
        ...
        private void ExecuteFetch(SqlTransaction tr, FilterCriteria criteria)
        {
            using (SqlCommand cm = tr.Connection.CreateCommand())
            {
                ...
                {
                    while (dr.Read())
                        this.Add(DieuChuyenNoiBoCCDC.GetDieuChuyenNoiBoCongCuDungCu(dr));
                }
            }//using
        }
        ...
        protected override void DataPortal_Update()
        {
            ...
            using (SqlConnection cn = new SqlConnection(Database.ERP_Connection))
            {
                ...
                try
                {
                    ...
                    foreach (DieuChuyenNoiBoCCDC deletedChild in DeletedList)
                        deletedChild.DeleteSelf(tr);
                    DeletedList.Clear();

                    // loop through each non-deleted child object
                    foreach (DieuChuyenNoiBoCCDC child in this)
                    {
                        if (child.IsNew)
                            child.Insert(tr);
                        else
