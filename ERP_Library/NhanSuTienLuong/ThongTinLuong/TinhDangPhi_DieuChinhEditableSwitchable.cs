...
{ 
	[Serializable()]
    public class TinhDangPhi_DieuChinh : Csla.BusinessBase<TinhDangPhi_DieuChinh>
	{
        ...
        private TinhDangPhi_DieuChinh()
		{ /* require use of factory method */ }

        public static TinhDangPhi_DieuChinh NewTinhDangPhi()
		{
			if (!CanAddObject())
				throw new System.Security.SecurityException("User not authorized to add a TinhDangPhi");
            return DataPortal.Create<TinhDangPhi_DieuChinh>();
		}

        public static TinhDangPhi_DieuChinh GetTinhDangPhi(int maTinhDangPhi)
		{
			if (!CanGetObject())
				throw new System.Security.SecurityException("User not authorized to view a TinhDangPhi");
            return DataPortal.Fetch<TinhDangPhi_DieuChinh>(new Criteria(maTinhDangPhi));
		}
            ...
            using (SqlConnection cn = new SqlConnection(Database.ERP_Connection))
            {
                ...
                try
                {
                    using (SqlCommand cm = cn.CreateCommand())
                    {
                        ...
                        cm.CommandTimeout = 0;
                        cm.CommandText = "spd_InserttblnsNhanVien_DangPhi_DieuChinh";
        ...
        public override TinhDangPhi_DieuChinh Save()
		{
		...
		internal static TinhDangPhi_DieuChinh NewTinhDangPhiChild()
		{
			TinhDangPhi_DieuChinh child = new TinhDangPhi_DieuChinh();
			child.ValidationRules.CheckRules();
			child.MarkAsChild();
			return child;
		}

		internal static TinhDangPhi_DieuChinh GetTinhDangPhi(SafeDataReader dr)
		{
            TinhDangPhi_DieuChinh child = new TinhDangPhi_DieuChinh();
			child.MarkAsChild();
			child.Fetch(dr);
		...
		private void ExecuteFetch(SqlTransaction tr, Criteria criteria)
		{
			using (SqlCommand cm = tr.Connection.CreateCommand())
			{
				cm.Transaction = tr;
				cm.CommandType = CommandType.StoredProcedure;
                cm.CommandText = "spd_SelecttblnsNhanVien_DangPhi_DieuChinh";
		...
		private void ExecuteDelete(SqlTransaction tr, Criteria criteria)
		{
            using (SqlCommand cm = tr.Connection.CreateCommand())
            {
                cm.Transaction = tr;
                cm.CommandType = CommandType.StoredProcedure;
                cm.CommandText = "spd_DeletetblnsNhanVien_DangPhi_DieuChinh";
		...
		#region Data Access - Insert
		internal void Insert(SqlTransaction tr, TinhDangPhi_DieuChinhList parent)
		{
			if (!IsDirty) return;
		...
		private void ExecuteInsert(SqlTransaction tr, TinhDangPhi_DieuChinhList parent)
		{
			using (SqlCommand cm = tr.Connection.CreateCommand())
			{
				cm.Transaction = tr;
		...
		private void AddInsertParameters(SqlCommand cm, TinhDangPhi_DieuChinhList parent)
		{
		...
		#region Data Access - Update
		internal void Update(SqlTransaction tr, TinhDangPhi_DieuChinhList parent)
		{
			if (!IsDirty) return;
		...
		private void ExecuteUpdate(SqlTransaction tr, TinhDangPhi_DieuChinhList parent)
		{
			using (SqlCommand cm = tr.Connection.CreateCommand())
			{
				cm.Transaction = tr;
				cm.CommandType = CommandType.StoredProcedure;
                cm.CommandText = "spd_UpdatetblnsNhanVien_DangPhi_DieuChinh";
		...
		private void AddUpdateParameters(SqlCommand cm, TinhDangPhi_DieuChinhList parent)
		{
			cm.Parameters.AddWithValue("@MaTinhDangPhi", _maTinhDangPhi);
			if (_maNhanVien != 0)
				cm.Parameters.AddWithValue("@MaNhanVien", _maNhanVien);
