...
{ 
	[Serializable()] 
	public class ChuongTrinhCon : Csla.BusinessBase<ChuongTrinhCon>
	{
        ...
        public string TenChuongTrinhCha
        {
            get
            {
                ...
                if(MaChuongTrinhCha!=0)
                {                    
                   tenchuongtrinhcha= ChuongTrinhCon.GetChuongTrinh(MaChuongTrinhCha).TenChuongTrinh;
                }
                return tenchuongtrinhcha;
		...
		private ChuongTrinhCon()
		{ /* require use of factory method */ }
        private ChuongTrinhCon(int maChuongTrinh, string tenChuongTrinh)
        { /* require use of factory method */
            this._maChuongTrinh = maChuongTrinh;
            this._tenChuongTrinh = tenChuongTrinh;
        }

        public static ChuongTrinhCon NewChuongTrinh( string tenChuongTrinh)
        {
            ChuongTrinhCon i = new ChuongTrinhCon();
            i._tenChuongTrinh = tenChuongTrinh;
            return i;
        }
		public static ChuongTrinhCon NewChuongTrinh()
		{
			if (!CanAddObject())
				throw new System.Security.SecurityException("User not authorized to add a ChuongTrinh");
			return DataPortal.Create<ChuongTrinhCon>();
		}

		public static ChuongTrinhCon GetChuongTrinh(int maChuongTrinh)
		{
			if (!CanGetObject())
				throw new System.Security.SecurityException("User not authorized to view a ChuongTrinh");
			return DataPortal.Fetch<ChuongTrinhCon>(new Criteria(maChuongTrinh));
		}
        public static ChuongTrinhCon GetChuongTrinhByDNTT(long maPhieu)
        {
            if (!CanGetObject())
                throw new System.Security.SecurityException("User not authorized to view a ChuongTrinh");
            return DataPortal.Fetch<ChuongTrinhCon>(new CriteriaByDNTT(maPhieu));
        }
		...
		public override ChuongTrinhCon Save()
		{
		...
		internal static ChuongTrinhCon NewChuongTrinhChild()
		{
			ChuongTrinhCon child = new ChuongTrinhCon();
			child.ValidationRules.CheckRules();
			child.MarkAsChild();
			return child;
		}

		internal static ChuongTrinhCon GetChuongTrinh(SafeDataReader dr)
		{
			ChuongTrinhCon child =  new ChuongTrinhCon();
			child.MarkAsChild();
			child.Fetch(dr);
		...
		#region Data Access - Insert
		internal void Insert(SqlTransaction tr, ChuongTrinh parent)
		{
			if (!IsDirty) return;
		...
		private void ExecuteInsert(SqlTransaction tr, ChuongTrinh parent)
		{
			using (SqlCommand cm = tr.Connection.CreateCommand())
			{
				cm.Transaction = tr;
		...
		private void AddInsertParameters(SqlCommand cm, ChuongTrinh parent)
		{
            ...
            if (parent.MaChuongTrinh != 0)
                cm.Parameters.AddWithValue("MaChuongTrinhCha", parent.MaChuongTrinh);
            else
		...
		private void ExecuteUpdate(SqlTransaction tr, ChuongTrinh parent)
		{
			using (SqlCommand cm = tr.Connection.CreateCommand())
			{
				cm.Transaction = tr;
		...
		private void AddUpdateParameters(SqlCommand cm, ChuongTrinh parent)
		{
			cm.Parameters.AddWithValue("@MaChuongTrinh", _maChuongTrinh);
			if (_maQL.Length > 0)
				cm.Parameters.AddWithValue("@MaQL", _maQL);
