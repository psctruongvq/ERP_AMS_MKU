...
namespace PSC_ERP
{
    public partial class FrmChungTuChiTienMat : XtraForm
    {
        #region Properties
        ...
        private void DesignGrid_ButToan()
        {
            ...
            HamDungChung.ShowFieldGridViewDev(gridView_ButToan, new string[] { "DienGiai", "NoTaiKhoan", "CoTaiKhoan", "SoTienNgoaiTe", "SoTien", "DoiTuongNo", "DoiTuongCo", "MaDonVi", "IDKhoanMuc", "MaHopDong", "SoHoaDonThamChieu", "MaNhanVien" },
                                new string[] { "Nội dung", "Nợ Tài Khoản", "Có Tài Khoản", "Số tiền ngoại tệ", "Số Tiền", "Đối tượng nợ", "Đối tượng có", "Đơn vị", "Thuộc khoản mục", "Số hợp đồng", "Số hóa đơn tham chiếu", "Mã Nhân Viên" },
        ...
        private void DesignGrid_HoaDon()
        {
            ...
            HamDungChung.ShowFieldGridViewDev_Modify(ChungTu_HoaDonThanhToangridView, new string[] { "HoaDon.GhiChu", "HoaDon.MauHoaDon", "HoaDon.KyHieuMauHoaDon", "HoaDon.SoHoaDon", "HoaDon.SoSerial", "HoaDon.NgayLap", "HoaDon.NhomHHDVMuaVao", "HoaDon.ThanhTien", "HoaDon.ThueSuatVAT", "HoaDon.ThueVAT", "HoaDon.TongTien", "SoTienThanhToan", "HoaDon.MaDoiTac", "HoaDon.TenDoiTac", "HoaDon.MaSoThue", "HoaDon.DiaChi" },
                                new string[] { "Diễn giải", "Mẫu hóa đơn", "Ký hiệu mẫu HĐ", "Số hóa đơn", "Số serial", "Ngày hóa đơn", "Nhóm hóa đơn", "Tiền trước thuế", "% thuế", "Tiền Thuế", "Tiền sau thuế", "Số tiền thanh toán", "Mã đối tượng", "Tên đối tượng", "Mã số thuế", "Địa chỉ" },
        ...
        private Boolean DuocPhepLuu()
        {
            ...
            //========Sua Theo Yeu Cau Cua THUONG   2019.07.16   ko nhap so tien van cho luu chung tu, nen ko kiem tra so tien cua chung tu PC nua.
            else if (_ChungTu.Tien.ThanhTien == 0)
            {
                if (MessageBox.Show(this, "Chứng từ chưa có hạch toán, chưa có số tiền, bạn có muốn tiếp tục lưu", "Thông Báo", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == System.Windows.Forms.DialogResult.No)
                {
                ...
                if (chitietbienlailistSelected.Count != 0)//M
                {
                    ...
                    foreach (ChiTietBienLaiFromOtherDB ctbienlai in chitietbienlailistSelected)
                    {
                        ...
                        if (_ChungTu.DinhKhoan.ButToan.Count > 0)
                        {
                            ...
                            {
                                if (
                                     (ctbienlai.MaBienLai != Guid.Empty && item.OidMaBienLai == ctbienlai.MaBienLai)
                                     ||
                                     (ctbienlai.MaBienLaiInt != 0 && item.IDBienLai == ctbienlai.MaBienLaiInt)
                                   )
                                {
                                    insert = false;
                                    break;
        ...
        public FrmChungTuChiTienMat(ChungTuCustomize chungtuCus)
        {
            InitializeComponent();
        ...
        public FrmChungTuChiTienMat()
        {
            InitializeComponent();
        }

        public FrmChungTuChiTienMat(ChungTu chungTu)
        {
        ...
        public FrmChungTuChiTienMat(long maChungTu)
        {
        ...
        private DataTable Table_DataRS(int maBoPhan)
        {
            ...
            using (SqlConnection cn = new SqlConnection(Database.ERP_Connection))
            {
                ...
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.Parameters.AddWithValue("@MaBoPhan", maBoPhan);
                    cm.Parameters.AddWithValue("@UserID", CurrentUser.Info.UserID);
                    cm.Parameters.AddWithValue("@MaCongTy", CurrentUser.Info.MaCongTy);
        ...
        private void dtmp_Ngay_Leave(object sender, EventArgs e)
        {
            ...
            {
                _ChungTu.SoChungTu = PublicClass.SetSoChungTuByMaLoaiChungTu(_LoaiChungTu.MaLoaiCT, dtmp_Ngay.DateTime.Date);
                _ChungTu.NgayThucHien = dtmp_Ngay.DateTime.Date;
            }
            else if (dtmp_Ngay.OldEditValue != dtmp_Ngay.EditValue && dtmp_Ngay.Text != "")
            {
                ...
                else if (dtmp_Ngay.OldEditValue == null || _ChungTu.NgayLap.Year <= 1900)
                {
                    _ChungTu.SoChungTu = PublicClass.SetSoChungTuByMaLoaiChungTu(_LoaiChungTu.MaLoaiCT, dtmp_Ngay.DateTime);
                }
        ...
        private void FrmChungTuChiTienMat_Load(object sender, EventArgs e)
        {
            this.ReadOnlyConTrolByStatus(this.strStatus);
