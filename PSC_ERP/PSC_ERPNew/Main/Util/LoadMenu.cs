...
namespace PSC_ERPNew.Main.Util
{
    public class LoadMenu
    {
      ...
      private int userID = 0;
        ...
        public static void LoadAllMenu(IWin32Window owner, RibbonControl ribbon)
        {
            if (ribbon != null)
            {
                while (ribbon.Pages.Count > 1)
            ...
            LoadMenu loadMenuObj = new LoadMenu(ribbon, owner);
            loadMenuObj.Load();
        ...
        public LoadMenu(RibbonControl ribbon, IWin32Window owner)
        {
            this._ribbon = ribbon;
            this._owner = owner;
        }
        ...
        public void Load()
        {
            ...
            if (_ribbon != null)
                InsertMenuInto(_ribbon, userID, isAdmin);//Gan menu vao ribbon         
        }
        #endregion

        #region Private Member Method
        private void InsertMenuInto(Object parent,int userID, bool isAdmin)
        {
            ...
            if (parent is RibbonControl)
            {
                ...
                foreach (AppMenu menu in menuList)
                {
                    ...
                    //
                    InsertMenuInto(page,  userID,  isAdmin);//goi de quy ->them group vao page
            ...
            else if (parent is RibbonPage)
            {
                ...
                foreach (mdl.AppMenu menu in menuList)
                {
                    ...
                    InsertMenuInto(pageGroup, userID, isAdmin);//goi de quy -> them barButtonItem vao group
                }
            }
            else if (parent is RibbonPageGroup)
            {
                RibbonPageGroup pageGroup = (RibbonPageGroup)parent;
                pageGroup.AllowTextClipping = true;
                pageGroup.AllowMinimize = true;                
            ...
            if (barItem != null)
            {
                ...
                if (menu != null && menu.AppFunction != null)
                {
                    ...
                    barItem.ItemClick += (object senderz, ItemClickEventArgs ez) =>
                    {
                        ...
                        {
                            Form frm = (Form)obj;
                            //kiểm tra form đã mở không được mở lại (1 form chỉ được xuất hiện 1 lần, 1 from được xuất hiện nhiều lần nếu có chức năng khác nhau)
                            frm.Tag = m.ID;
                            Form main= (Form)_owner;
                            foreach (Form o in main.MdiChildren)
                            {
                                if (o.Tag != null)
                                {
                                    if ((Int32)o.Tag == m.ID)
        ...
        private void GoiDeQuyThemMenuVaoContainer(mdl.AppMenu menu, BarItem barItem)
        {
            if (menu.Type == "BarSubItem"
                ...
                InsertMenuInto(barItem, userID, isAdmin);//goi de quy
        }
