...
namespace PSC_ERPNew.Main.PhanHe.TSCD.Reports
{
    ...
    {
        public bool TongHopKinhPhiMuaSamTaiSanTangTrongKyTheoDonViSuDung()//M
        {
            ...
            ReportHelper.ShowReport(key, () =>
            {
                ...
                dataAccess.FillDataSet(ref dataSet, "MainData", "spRpt_TSCD_TongHopKinhPhiMuaSamTaiSanTangTrongKyTheoDonViSuDung", "@TuNgay,@DenNgay,@NguonMua", tuNgay, denNgay, nguonMua);
                #region Extend table
            ...
            dataAccess.FillDataSet(ref dataSet, "MainData", "spRpt_TSCD_TongHopKinhPhiMuaSamTaiSanTangTrongKyTheoDonViSuDung_New", "@TuNgay,@DenNgay,@NguonMua,@LoaiBaoCao", tuNgay, denNgay, nguonMua, LoaiBaoCao);

            frmReportingViewer frm = new frmReportingViewer();
            frm.MainReportView.LocalReport.DataSources.Clear();
            frm.MainReportView.LocalReport.ReportEmbeddedResource = "PSC_ERP.PSC_ERPNew.Main.PhanHe.TSCD.Reports.rptTongHopKinhPhiTangTrongKyTheoPhongBan_17.rdlc";
            //người ký tên
            frm.SetNguoiKy(100);//lấy trường hợp default truyền số bất kỳ không cho case là được (case từ 0-3)

            frm.MainReportView.LocalReport.DataSources.Add(new Microsoft.Reporting.WinForms.ReportDataSource("BaoCao_17", dataSet.Tables[0]));
