...
{
    public partial class frmTaiSuDungTSCDCaBiet : XtraForm
    {
        //Singleton
        #region Singleton
        private static frmTaiSuDungTSCDCaBiet singleton_ = null;
        public static frmTaiSuDungTSCDCaBiet Singleton
        {
            get
            {
                if (singleton_ == null || singleton_.IsDisposed)
                    singleton_ = new frmTaiSuDungTSCDCaBiet();
                return singleton_;
            }
        }
       ...
       // List<tblnsBoPhan> _boPhan_LichSuDCList = null;
        ...
        public frmTaiSuDungTSCDCaBiet()
        {
            InitializeComponent();
        }

        public frmTaiSuDungTSCDCaBiet(int maNVNgungvaTaiSuDungTSCD)
        {
            InitializeComponent();
        ...
        private void XemLaiChungTuCu(int maNVNgungvaTaiSuDungTSCD)
        {
            ...
            GanBindingSource();
            TabTaiSuDungTSCaBiet.SelectedTabPageIndex = 1;
        }
        ...
        private bool DuocPhepLuu()
        {
            ...
            if (this._nVNgungvaTaiSuDungTSCD.tblCT_NVNgungvaTaiSuDungTSCD.Count == 0)
            {
                ...
                TabTaiSuDungTSCaBiet.SelectedTabPageIndex = 0;
                return false;
            }
                        ...
                        if (radioDuyet.SelectedIndex > -1)
                        {
                            ...
                            if ((Int32)radioDuyet.EditValue == 0)
                            {
                                ...
                                if (_user.GroupID == 38)
                                {
                                   ...
                                   SendEmail_DKAPP(emailNhan, emailGui, matKhauEmailGui, String.Empty, 587, $"Không Duyệt Chứng Từ {_nVNgungvaTaiSuDungTSCD.SoChungTu}", "Chứng từ tái sử dụng TSCĐ:" + _nVNgungvaTaiSuDungTSCD.SoChungTu + " không được duyệt! Với lý do: " + _nVNgungvaTaiSuDungTSCD.LyDo);
                                }
                                else
                                {
                                    _nVNgungvaTaiSuDungTSCD.TinhTrang = null;
                                   SendEmail_DKAPP(emailNhan, emailGui, matKhauEmailGui, String.Empty, 587, $"Đề Nghị Duyệt Chứng Từ {_nVNgungvaTaiSuDungTSCD.SoChungTu}", "Đề nghị duyệt chứng từ tái sử dụng TSCĐ có số:" + _nVNgungvaTaiSuDungTSCD.SoChungTu + " với nội dung:" + _nVNgungvaTaiSuDungTSCD.GhiChu);
                                }
                            }
                            else
                            {
                                ...
                                _nVNgungvaTaiSuDungTSCD.HoanTat = duyet;
                                SendEmail_DKAPP(emailNhan, emailGui, matKhauEmailGui, String.Empty, 587, $"Đã Duyệt Chứng Từ {_nVNgungvaTaiSuDungTSCD.SoChungTu}", "Chứng từ tái sử dụng TSCĐ có số:" + _nVNgungvaTaiSuDungTSCD.SoChungTu + " đã được duyệt!");
                                foreach (tblCT_NVNgungvaTaiSuDungTSCD ct in _nVNgungvaTaiSuDungTSCD.tblCT_NVNgungvaTaiSuDungTSCD)
                                {
                                    ct.tblTaiSanCoDinhCaBiet.NgungSuDung = false;
                                    ct.tblTaiSanCoDinhCaBiet.TinhTrang = "Tái Sử Dụng";
                                }
                            }
                        }
                        else
                        {
                           SendEmail_DKAPP(emailNhan, emailGui, matKhauEmailGui, String.Empty, 587, $"Đề Nghị Duyệt Chứng Từ {_nVNgungvaTaiSuDungTSCD.SoChungTu}", "Đề nghị duyệt chứng từ tái sử dụng TSCĐ :" + _nVNgungvaTaiSuDungTSCD.SoChungTu + " với nội dung:" + _nVNgungvaTaiSuDungTSCD.GhiChu);
                        }
                        tblNVNgungvaTaiSuDungTSCDBindingSource.EndEdit();
                        // Lưu nghiệp vụ điều chuyển nội bộ
                        this._nVNgungvaTaiSuDungTSCD_Factory.SaveChanges(BusinessCodeEnum.TSCD_TaiSuDung.ToString());
                    }
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            ...
            //tìm
            // comment out the selected lines 11/11/2020
             //_taiSanCDCaBietList = TaiSanCoDinhCaBiet_Factory.New().Context.spd_TSCD_DanhSachTaiSanCoDinhHayCCDCCoDenNgay_TheoPhongBanViTriLoaiTaiSanNgungSuDung
            // (DateTime.Now.Date, maPhongBanDangChon, maViTri, maTSCDDangChon, _user.MaCongTy, 1).ToList();
            //DuyetTSCD_Factory.New().Get_DanhSachDuyetTSCDCaBietDaDuyet_PhucVuDieuChuyenNoiBo(maPhongBanDangChon, maTSCDDangChon).ToList()                                                                                                                                                                                                                                         // Đưa dữ liệu vào bindingsource
            // tblTaiSanCoDinhCaBietBindingSource.DataSource = _taiSanCDCaBietList;
                ...
                Nullable<System.DateTime> denNgay, Nullable<int> maBoPhan, Nullable<int> maViTri, Nullable<int> maLoai, Nullable<int> maCongTy, Nullable<int> loaiBaoCao)
        {
            ...
            using (SqlConnection cn = new SqlConnection(Database.ERP_Connection))
            {
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandText = "spd_TSCD_DanhSachTaiSanCoDinhHayCCDCCoDenNgay_TheoPhongBanViTriLoaiTaiSanNgungSuDung";
        ...
        private void frmDieuChuyenNoiBoTSCDCaBiet_Load(object sender, EventArgs e)
        {
          ...
          //  _boPhan_LichSuDCList.Insert(0, boPhan_ChonTatCa);
            ...
            else
            {
                ...
                _nVNgungvaTaiSuDungTSCD = _nVNgungvaTaiSuDungTSCD_Factory.NewNVNgungVaTaiSuDung_TaiSuDung();
            }
            ...
            GridUtil.DeleteHelper.Setup_ManualType(gridView_CTNgungVaTaiSuDung, (GridView gridView, List<Object> deleteList) =>
            {
                ...
                CT_NVNgungvaTaiSuDungTSCD_Factory.FullDeleteCT_NVTaiSuDungTSCD(context: _nVNgungvaTaiSuDungTSCD_Factory.Context, deleteList: deleteList);
            });
        ...
        private void btnDuaTaiSanVaoChungTu_Click(object sender, EventArgs e)
        {
            ...
            //            chiTietNVNgungvaTaiSuDungTSCD.tblTaiSanCoDinhCaBiet.NgungSuDung = false;
            //        }
            //    }
            ...
            try
            {
                ...
                using (DialogUtil.Wait())
                {
                    if (grdViewTSCDCaBiet.RowCount > 0)
                    {
                        ...
                        // Xóa tài sản cố định cá biệt vừa chọn ra khỏi danh sách tìm
                        RemoveTaiSanCoDinhDaChonRaKhoiDanhSachTim();
                        TabTaiSuDungTSCaBiet.SelectedTabPage =  xtraTabPage1;
                    }
                }
                //thông báo người dùng nếu chưa chọn bất cứ dòng nào từ danh sách duyệt
            ...
            else
            {
                if (_nVNgungvaTaiSuDungTSCD.UserID == _user.UserID && DialogResult.Yes == DialogUtil.ShowYesNo(String.Format("Bạn muốn xóa chứng từ [{0}]?", this.Text)))
                {
                    try
                    {
                        ...
                        _nVNgungvaTaiSuDungTSCD_Factory.FullDeleteNghiepVu_TaiSuDung(_nVNgungvaTaiSuDungTSCD);
        ...
        {
            frmDanhSachTaiSuDungTSCDCaBiet.ShowSingleton(null, this.MdiParent);
        }
        ...
        private void frmTaiSuDungTSCDCaBiet_FormClosing(object sender, FormClosingEventArgs e)
        {
            txtBlackHole.Focus();
        ...
        private void barButtonItem1_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            {                                                //99 là Mã TSCD Tái sử dụng không có trong tblLoaiChungTu
                frmNTSD_ChonFilesUpLoad frm_upFile = new frmNTSD_ChonFilesUpLoad(_nVNgungvaTaiSuDungTSCD.MaNghiepVu,99);
        ...
        private void deNgayLap_EditValueChanged(object sender, EventArgs e)
        {
            ...
            if (ngayChungTu != new DateTime(1, 1, 1) && deNgayLap.DateTime.Date != _nVNgungvaTaiSuDungTSCD.NgayChungTu)
            {
                ...
                if (_nVNgungvaTaiSuDungTSCD.MaNghiepVu == 0)
                    _nVNgungvaTaiSuDungTSCD.SoChungTu = NVNgungvaTaiSuDungTSCD_Factory.New().Get_NextSoChungTu_ByYear_And_Month_TaiSuDung(_nVNgungvaTaiSuDungTSCD.NgayChungTu.Year, _nVNgungvaTaiSuDungTSCD.NgayChungTu.Month, PSC_ERP_Global.Main.UserID);
