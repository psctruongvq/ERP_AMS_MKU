...
--DBCC CHECKIDENT ('dbo.tblHangHoa', RESEED, 15000)
  ...
  FROM TSHTV.dbo.tblCCDC AS ccdc
...
declare @Cur cursor
			set @Cur=cursor for
				SELECT MaTSCDCaBiet FROM TSHTV.dbo.tblCCDC
			open @Cur
			...
			while (@@fetch_status=0)
				...
				---------------------------------------------
					INSERT INTO dbo.tblCongCuDungCu
					  ...
					  SELECT hh.TestModel
						, ccdc.NguyenGiaMua + (SELECT ISNULL(GiaTriTang,0)-ISNULL(GiaTriGiam,0) FROM TSHTV.dbo.tblChiTietDieuChinhGiaTri WHERE MaTSCDCaBiet=@MaTSCDCaBiet) 
							+ (SELECT GiaTri FROM TSHTV.dbo.v_BoSungDCPT WHERE MaTSCDCaBiet=@MaTSCDCaBiet)
						, ccdc.SoHieu
						,ccdc.NgayNhan
						,0--ThanhLy
						,NULL--NgayThanhLy
						...
						,ISNULL(ccdc.LuyKeKhauHao,0) + ISNULL(ccdc.LuyKeHaoMon,0)--ChiPhiDaPhanBo
						,ISNULL(ccdc.LuyKeKhauHao,0) + ISNULL(ccdc.LuyKeHaoMon,0)--ChiPhiPBLanDau
					   ...
					   FROM TSHTV.dbo.tblCCDC AS ccdc
						...
						WHERE ccdc.MaTSCD = @MaTSCDCaBiet
